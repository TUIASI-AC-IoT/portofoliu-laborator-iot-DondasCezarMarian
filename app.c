...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_system_boot_id:
      ...
      sl_bt_sm_set_passkey(9090);

      sl_bt_sm_set_bondable_mode(1);
      break;

    case sl_bt_evt_sm_passkey_display_id:
      app_log("Pass key : %d\r\n", evt->data.evt_sm_passkey_display);
      break;

    case sl_bt_evt_connection_parameters_id:
      app_log("security_mode info: %d\r\n", evt->data.evt_connection_parameters);
      break;

    case sl_bt_evt_sm_bonded_id:
      app_log("Success message: %d\r\n", evt->data.evt_sm_bonded);
      break;

    case sl_bt_evt_sm_bonding_failed_id:
      app_log("Failed message: %d\r\n", evt->data.evt_sm_bonding_failed);
      break;
